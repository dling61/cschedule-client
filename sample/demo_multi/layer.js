var layersample = {
        ...
        conversationList: [],
        openConversationUrlList: [],
        openConversationUrlIndexList: [],
        msgNumList: [],
    ...
    function addParticipant(conversationUrl, userId) {
        return $.ajax({
            url: conversationUrl,
            method: "PATCH",
            headers: {
            ...
            statusCode: {
                404: function() {
                  alert("conversation does not exist anymore");
                }
            }
        });
    }
    ...
    function removeParticipant(conversationUrl, userId) {
        return $.ajax({
            url: conversationUrl,
            method: "PATCH",
            headers: {
    ...
    function checkConExist(conversations, con_title) {
        for(var i=0; i<conversations.length; i++) {
            if(conversations[i].metadata.title == con_title) {
                layersample.conversationIndex = i;
                return true;
            }
        }
    ...
    function getMessages(conversationUrl) {
        return $.ajax({
            ...
            statusCode: {
                404: function() {
                  console.log("conversation does not exist anymore");
                }
            }
        })
    }
