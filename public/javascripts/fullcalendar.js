...
var Grid = fc.Grid = RowRenderer.extend({
	...
	bgCellHtml: function(cell) {
        ...
        return '<td class="' + classes.join(' ') + '"' +
			' data-date="Dongling"' + // if date has a time, won't format it
			'></td>';
        /*
		return '<td class="' + classes.join(' ') + '"' +
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		titleHtml =
			'<span class="fc-title">' + //WFB '<br><img src=".\\images\\Irene.png"  height="22" width="22">' +
				//WFB (htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
